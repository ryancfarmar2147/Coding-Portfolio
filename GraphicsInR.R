...
DavisPlot + geom_point(shape=0, size=2)
...
ggplot(uspopage, aes(x=Year, y=Thousands)) + geom_line(aes(color=AgeGroup)) + geom_point(shape=21)
...
bar2 + stat_summary(fun.y=mean, geom="bar", fill="white", colour="black") +
...
midwest <- midwest
midwest_plot <- ggplot(midwest, aes(x=state,fill=factor(inmetro))) 
midwest_plot + geom_bar(position="stack")
...
y <- quantile(faithful2$waiting, c(0.25, 0.75)) 
x <- qnorm( c(0.25, 0.75))  
slope <- diff(y)/diff(x) 
int <- y[1] - slope*x[1]   
...
DavisScatter2 + labs(title="Weights of individuals who regularly exercise",
                     x="Measured weight", y="Reported weight")  + labs(color = "Gender") +
  scale_colour_discrete(limits=c("M","F")) + scale_color_manual(values = c("M"= "green","F"="red"))
...
##Limits and labels can be used together to produce a graphic that accurately represents data##
